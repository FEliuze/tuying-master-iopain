...
报错里的 **端口号**（如 `:80`）是 **K8S 探针要连的端口**，必须和进程实际监听口**完全一致**。

1. 仅把环境变量改成 `PORT=8080` **不够**：若探活/容器端口/Service 仍指向 **80**，日志仍会出现 **`dial ... :80` `connection refused`**（没有进程在 80 上监听时）。
2. 在云托管控制台把 **容器暴露端口、健康检查（readiness/liveness）端口、环境变量 PORT** 全部设为同一值；推荐 **8080**（与镜像默认一致，且非特权端口；部分环境非 **root** 不能绑定 **80**，会启动失败或拒绝连接）。
3. 进程须监听 `0.0.0.0`（本镜像已 `--host=0.0.0.0`），勿只绑 `127.0.0.1`。
4. 首启拉模型较慢时，可适当**增大探活 initialDelay/超时**，避免尚未 listen 就判定失败。
